...
private:
    ...
    static bool ParseVersionFromString(
        const char* ptr,
        int& major,
        int& minor,
        int& revision,
        int& build)
    {
        return sscanf_s(
                   ...
                   &revision,
                   &build) == 4 &&
               ((major != 1 && major != 0) || minor != 0 || revision != 0 ||
                build != 0);
    }
...
public:
    ...
    bool FindIdByOffset(unsigned long long offset, unsigned long long& result)
        const
    {
    ...
    bool GetExecutableVersion(int& major, int& minor, int& revision, int& build)
        const
    {
        ...
        DWORD verHandle = 0;
        // Note: Why are these lines even here?
        // UINT size = 0;
    ...
    void GetLoadedVersion(int& major, int& minor, int& revision, int& build)
        const
    {
        major = _ver[0];
        minor = _ver[1];
    ...
    bool Load(int major, int minor, int revision, int build)
    {
        ...
        {
            HMODULE handle = GetModuleHandleA(
                _moduleName.empty() ? NULL : _moduleName.c_str());
